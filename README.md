...
```ShellSession
# Copy ``inventory/sample`` as ``inventory/german_cluster``
cp -rfp inventory/sample inventory/german_cluster

# Update Ansible inventory file with inventory builder
declare -a IPS=(10.10.1.3 10.10.1.4 10.10.1.5)
CONFIG_FILE=inventory/german_cluster/hosts.yaml python3 contrib/inventory_builder/inventory.py ${IPS[@]}

# Review and change parameters under ``inventory/german_cluster/group_vars``
cat inventory/german_cluster/group_vars/all/all.yml
cat inventory/german_cluster/group_vars/k8s_cluster/k8s-cluster.yml
...
ansible-playbook -i inventory/german_cluster/hosts.yaml  --become --become-user=root reset.yml

# Deploy Kubespray with Ansible Playbook - run the playbook as root
# The option `--become` is required, as for example writing SSL keys in /etc/,
# installing packages and interacting with various systemd daemons.
# Without --become the playbook will fail to run!
ansible-playbook -i inventory/german_cluster/hosts.yaml  --become --become-user=root cluster.yml
```
